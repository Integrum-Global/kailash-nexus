...
"""
            ...
            try:
                ...
                if "outputs" in result:
                    analyze_result = (
                        result.get("outputs", {}).get("analyze", {}).get("result", {})
                    )
...
"""
            ...
            # Use unique port for E2E test
            n = Nexus(
                api_port=9012, auto_discovery=False
            )  # Disable auto-discovery for test
...
"""
            ...
            try:
                ...
                response = requests.get("http://localhost:9012/health")
                assert response.status_code == 200
                health = response.json()
                assert health["status"] == "healthy"

                # 6. Custom health workflow available
                response = requests.post("http://localhost:9012/workflows/health_check")
                assert response.status_code == 200
                result = response.json()
                if "outputs" in result:
                    check_result = (
    ...
    def test_devops_zero_config_deployment(self, docker_env):
        ...
        # Use unique port for E2E test
        n = Nexus(api_port=9013, auto_discovery=False)
        ...
        try:
            ...
            response = requests.get("http://localhost:9013/health")
            assert response.status_code == 200

            # 3. Docs available
            response = requests.get("http://localhost:9013/docs")
            assert response.status_code == 200
            ...
            response = requests.post(
                "http://localhost:9013/workflows/deployment-status"
            )
            assert response.status_code == 200
            result = response.json()
            if "outputs" in result:
...
class TestAIDeveloperFlow:
    ...
    def test_ai_developer_mcp_integration(self, docker_env):
        ...
        # Use unique port for E2E test (skip MCP for now, test via API)
        n = Nexus(api_port=9014, auto_discovery=False)
...
"""
        ...
        try:
            ...
            response = requests.post(
                "http://localhost:9014/workflows/analyze-sentiment",
    ...
    def test_ai_developer_tool_composition(self, docker_env):
        ...
        # Use unique port for E2E test
        n = Nexus(api_port=9015, mcp_port=3011, auto_discovery=False)
...
"""
        ...
        try:
            ...
            response = requests.get("http://localhost:9015/workflows")
            ...
            summary_response = requests.post(
                "http://localhost:9015/workflows/summarize-text",
                json={"parameters": {}},  # Empty parameters since we use hardcoded data
            )
            ...
            keywords_response = requests.post(
                "http://localhost:9015/workflows/extract-keywords",
                json={"parameters": {}},  # Empty parameters since we use hardcoded data
            )
